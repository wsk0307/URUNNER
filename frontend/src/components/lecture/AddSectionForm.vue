    ...
    <v-btn class="d-block" @click="addSecttion = !addSecttion">add section</v-btn>
        <v-list dense>
          <v-list-item v-for="(section, index) in sectionList" :key="index">
            <v-list-item-title @click="goToUploadLecture(section.id)">
  ...
  methods: {
    registerSection() {
      const data = {
        id: this.id,
        topic: this.topic,
        lectureId: '임시번호', // 강의 현황에서 강의 리스트중 특정 리스트의 커리큘럼 관리 버튼을 클릭했을 때 나오는 것으로 id를 파라미터도 받음
      }
    ...
    getLectureSection() {
      axios.post('http://localhost:7777/mypage/lecture/getLectureSection')
          .then(res => {
            console.log(res.data);
            this.$store.commit('saveSectionList', res.data);
